...
/*function getCommandsToGenerateScripts() {
    ...
    for (var index=0; index<commands.length; index++) {
        console.log("getCommandTarget: "+ commands[index].innerHTML);
...
function getCommandsToGenerateScripts() {
    ...
    for (var index = 0; index < commands.length; index++) {
        ...
        let commandTarget = getCommandTarget(commands[index]); 
        let commandValue = getCommandValue(commands[index]);
        let locators = getLocatorsFromCommand(commands[index]);

        ret.push(new Command(commandName, commandTarget, commandValue, locators));
    }
    //console.log("Commands: "+ JSON.stringify(ret));

    return ret;
}

function getLocatorsFromCommand(commandElement) {
    let locators = [];
    let datalist = commandElement.querySelector('datalist');

    if (datalist) {
        let options = datalist.querySelectorAll('option');
        options.forEach(option => {
            locators.push(option.value);
        });
    }
